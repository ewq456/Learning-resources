...
- 查看数据类型： cout << "T2的类型为： " << typeid(T2).name() << endl;
...
* 函数模板利用关键字 template
* 使用函数模板有两种方式：自动类型推导mySwap(a, b);        显示指定类型mySwap<int>(a, b);
...
```C++
//利用模板提供通用的交换函数，<>里面的class可以替换为template，作用一样
template<class T>
void mySwap(T& a, T& b)
{
	T temp = a;
...
* `string& assign(int n, char c);`                   //用==n个字符c==赋给当前字符串
...
* rfind是从右往左查找第一次出现的位置，find是从左往右查找第一次出现的位置，但坐标依旧是从左往右计算
* `int rfind(const string& str, int pos = npos) const;`      //查找str最后一次位置,从pos开始查找
* `int rfind(const char* s, int pos = npos) const;`              //查找s最后一次出现位置,从pos开始查找
* `int rfind(const char* s, int pos, int n) const;`              //从pos查找s的前n个字符最后一次位置
* `int rfind(const char c, int pos = 0) const;  `                      //查找字符c最后一次出现位置
* `string& replace(int pos, int n, const string& str); `       //替换从pos开始n个字符为字符串str，n不够则以字符串实际长度为准
...
* `vector(v.begin(), v.end());   `       //将==v[begin(), end())区间==中的元素拷贝给本身。（前闭后开）
...
void test02()
{
	...
	//收缩内存
	vector<int>(v).swap(v); //匿名对象，匿名对象使用完后会被系统回收
...
* 双端数组，可以对头端或尾端进行插入删除操作



**deque与vector区别：**

* ==vector对于头部的插入删除效率低，越往前，需要后移的越多，数据量越大，效率越低==
...
队列中**只有队头和队尾才可以被外界使用**，因此**队列不允许有遍历行为**
...
//greater和less是std实现的两个仿函数（就是使一个类的使用看上去像一个函数。其实现就是类中实现一
```
...
* 对list容器中数据进行存取，list容器的迭代器是双向迭代器，不支持随机访问
...
* 所有不支持随机访问迭代器的容器，不可以用标准算法，但容器内部会提供对应一些算法
...
### 3.9 map/ multimap/ unorder_map容器
...
因此在需要使用字典结构进行算法编程的大部分情况下，都需要使用`unordered_map`而不是`map`。
...
![哈希表1](assets/20210104234805168.png)
...
void test01()
{
	...
	vector<bool> v2;
	v2.resize(v.size());  //此处必须先开辟空间，否则没有空间报错
...
- `set_difference(iterator beg1, iterator end1, iterator beg2, iterator end2, iterator dest);  `
  ...
  <img src="assets/image-20210930204538934.png" alt="image-20210930204538934" style="zoom:67%;" />
